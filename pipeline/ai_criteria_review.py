...
AI_CRITERIA_REVIEW_PROMPT = (
    ...
    "Критерии могут быть написаны на русском, английском или другом языке таблицы.\n"
    "Английский язык сам по себе не является проблемой: не помечай критерий problem только из-за языка.\n"
    "Для английских критериев допустимы base verb/action-list формулировки вроде 'Estimate, add, subtract...', "
