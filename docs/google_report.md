...
The `Ссылка на таблицу` column is shown as `Открыть` with a real cell hyperlink, so assistant principals can open the source class spreadsheet directly from the report.
